...
#nTraitRange=c(0,1,2,3,4);nTraits=3;nAgents=100;xDim=10;yDim=10;timeSteps=1000;innovationRate=0.01;replacementRate=0.1;interactionRadius=1;moveDistance=1;prestigeIndex=1
                ...
                if (transmissionType=="encounter"|transmissionType=="prestige")
                    {
                        ...
                        if (transmissionType=="prestige")
                            {
                                p=Agents[,prestigeIndex]
                            }
                        
                        targetAgentIndex<-apply(distMat,1,neighbourChooser,p=p,radius=interactionRadius) #select random neighbour
...
neighbourChooser<-function(x,p,radius)
    {
        finalIndex=NA
        if (any(x<radius,na.rm=TRUE))
            {
                ...
                probs=p[index]+1
                if(length(index)>1)
                    {finalIndex=sample(as.numeric(index),size=1,prob=probs)}
                if(length(index)==1)
                    {finalIndex=index}
            }
        return(finalIndex)
